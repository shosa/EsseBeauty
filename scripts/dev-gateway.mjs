...
// forwards to apps/api, apps/communications, apps/loyalty-marketing, or
// apps/booking by path, so a local `pnpm run dev` doesn't need Docker to see
// the four backends as one origin.
...
// Sale checkout is the one path under /appointments/ that stays on the core
// API (it's what actually completes an appointment — see
// routes/sales/index.ts) — checked before the booking patterns so it isn't
// shadowed by them.
const API_OVERRIDE_PATTERNS = [
...
function targetPortFor(url) {
  if (API_OVERRIDE_PATTERNS.some((pattern) => pattern.test(url))) return API_TARGET;
...
server.listen(PORT, () => {
  console.log(`[dev-gateway] listening on :${PORT} -> api::${API_TARGET}, communications::${COMMUNICATIONS_TARGET}, loyalty-marketing::${LOYALTY_MARKETING_TARGET}, booking::${BOOKING_TARGET}`);
});
