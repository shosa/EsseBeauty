...
export function WarehouseProducts({ items, query, lowOnly, itemType, selected, onQuery, onLowOnly, onItemType, onSelect, onSelectAll, onOpenOperation }: { items: WarehouseProduct[]; query: string; lowOnly: boolean; itemType: WarehouseItemType | "all"; selected: string[]; onQuery(value: string): void; onLowOnly(value: boolean): void; onItemType(value: WarehouseItemType | "all"): void; onSelect(id: string): void; onSelectAll(): void; onOpenOperation(mode: "adjustment" | "waste" | "revaluation" | "issue" | "count", product?: WarehouseProduct): void }) {
  ...
  return (
    <Card bodyClassName="p-0">
      ...
      {selected.length > 0 && (
        <div className="mx-4 mt-3 flex flex-nowrap items-center justify-between gap-3 rounded-xl border border-[#ead1df] bg-[#fffafd] px-4 py-2.5 text-sm">
          <span className="shrink-0"><strong>{selected.length}</strong> articoli selezionati</span>
          <div className="flex shrink-0 flex-nowrap gap-2"><Button className="whitespace-nowrap" onClick={() => onOpenOperation("waste")} size="sm" variant="outline"><PackagePlus className="size-3.5" />Registra scarto</Button><Button className="whitespace-nowrap" onClick={() => onOpenOperation("revaluation")} size="sm" variant="outline">Rivaluta costo medio</Button></div>
        </div>
      )}
      {items.length === 0 ? (
          ...
          <table className="w-full min-w-[1020px] text-left text-sm">
            <thead className="bg-[#faf7f9] text-[10px] font-black uppercase tracking-[.12em] text-stone-500">
              <tr>
                ...
                <th className="px-3">Articolo</th><th className="px-3">Tipo</th><th className="px-3">Scorta</th><th className="px-3 text-right">Prezzo</th><th className="px-3 text-right">Margine</th><th className="px-3">Fornitore</th><th className="w-16 pr-5" />
              </tr>
            </thead>
            <tbody>
              {items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-3"><div className="font-bold text-stone-900 group-hover:text-[#792f59]">{item.name}</div><div className="text-xs text-stone-500">{[item.brand, item.sku || "SKU non impostato", item.barcode].filter(Boolean).join(" · ")}</div></td>
                    <td className="px-3"><span className="rounded-full bg-stone-100 px-2 py-1 text-[10px] font-bold text-stone-600">{types[item.itemType]}</span></td>
                    <td className="px-3"><span className={`tnum font-black ${low ? "text-[#b23a2e]" : "text-stone-900"}`}>{item.stockQuantity} {item.unit}</span><div className="text-[11px] text-stone-400">soglia {item.lowStockThreshold}</div></td>
                    <td className="px-3 text-right tnum font-semibold text-stone-700">{euro(item.unitPriceCents)}</td>
                    <td className={`px-3 text-right tnum font-bold ${margin < 0 ? "text-[#b23a2e]" : "text-[#1c7a5c]"}`}>{euro(margin)}</td>
                    <td className="px-3 text-stone-600">{item.preferredSupplier || item.supplier || "—"}</td>
                    <td className="pr-5 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button aria-label={`Rettifica ${item.name}`} className="grid size-8 place-items-center rounded-lg text-stone-400 transition hover:bg-stone-100 hover:text-stone-700" onClick={(event) => { event.stopPropagation(); onOpenOperation("adjustment", item); }} type="button"><MoreHorizontal className="size-4" /></button>
