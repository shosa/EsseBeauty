...
export function WarehouseWorkspace() {
  ...
  async function addMovement(delta: number, reason: string) {
    await fetch(`${api}/api/salons/${salon?.id}/inventory/${movement?.id}/movements`, { method: "POST", credentials: "include", headers: { "content-type": "application/json" }, body: JSON.stringify({ delta, reason }) });
  ...
  return <AppPage maxWidth="max-w-[1600px]">
    <PageHeaderMetrics
      actions={<div className="flex flex-wrap gap-2"><Link aria-label="Nuovo articolo" className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-[#402334] bg-[linear-gradient(135deg,#402334_0%,#792f59_58%,#b85888_100%)] px-4 py-2.5 font-semibold text-white shadow-[0_16px_36px_rgb(121_47_89_/_0.28)] transition hover:-translate-y-0.5" href="/inventory/new"><Archive className="size-4" />Articolo</Link><Button aria-label="Carico" onClick={() => setActiveTab("movements")} size="sm" variant="outline"><ArrowDownToLine className="size-4" />Carico</Button><Button aria-label="Scarico" onClick={() => setActiveTab("movements")} size="sm" variant="outline"><ArrowUpFromLine className="size-4" />Scarico</Button><Button aria-label="Inventario" onClick={() => setActiveTab("counts")} size="sm" variant="outline"><ClipboardList className="size-4" />Inventario</Button><Button aria-label="Importa" onClick={() => setActiveTab("products")} size="sm" variant="outline"><FileUp className="size-4" />Importa</Button></div>}
      eyebrow="Magazzino"
