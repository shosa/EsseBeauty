...
import { Ban, CalendarClock, CalendarDays, Check, ChevronDown, MapPinned, ShieldCheck, Smartphone, UserRound } from "lucide-react";
import { APPOINTMENT_STATUS_PALETTE, appointmentStatusLabel, WEEK_DAYS_IT, type WorkingHours } from "@esse-beauty/shared";
import { AppPage, Breadcrumbs, Button, ConfirmDialog, DateField, designTokens, FormField, PageTransition, SaveActionButton, SaveToast, ScheduleEditor, Switch } from "@esse-beauty/ui";
import { useAuth } from "../../../../lib/auth-context";
import { staffStatusAction } from "../staff-status-action";

const api = process.env.NEXT_PUBLIC_API_URL ?? "";

type TabKey = "profile" | "agenda" | "access" | "location" | "hours";
...
function minutesOfPeriod(value: string) {
...
function StaffAgendaTimeline({
  ...
  const eventMinutes = [...appointments, ...blocks].flatMap((item) => [minutesOfDay(item.starts_at), minutesOfDay(item.ends_at)]);
  ...
  function itemPosition(startsAt: string, endsAt: string) {
    return position(minutesOfDay(startsAt), minutesOfDay(endsAt), 32);
  }
  ...
  return (
    <div className="grid grid-cols-[52px_1fr] overflow-hidden rounded-xl border border-stone-200">
      <div className="relative border-r border-stone-200 bg-[#faf9f7]" style={{ height }}>
        {hours.map((hour, index) => (
          <span className="absolute left-0 right-0 pr-2 text-right text-[10px] font-black text-stone-400" key={hour} style={{ top: index === 0 ? 4 : index === hours.length - 1 ? height - 16 : (hour - startHour) * hourHeight - 6 }}>
      ...
      <div className="relative bg-white" style={{ height }}>
        ...
        {gaps.map((gap) => (
          <div className="absolute left-0 right-0 flex items-center justify-center overflow-hidden border-y border-stone-300/70" key={`${gap.from}-${gap.to}`} style={{ ...position(gap.from, gap.to), background: "repeating-linear-gradient(135deg, rgba(120,113,108,.07) 0, rgba(120,113,108,.07) 8px, rgba(120,113,108,.16) 8px, rgba(120,113,108,.16) 10px)" }}>
            <span className="rounded-full bg-white/90 px-2.5 py-0.5 text-[8px] font-black uppercase tracking-[.12em] text-stone-500 shadow-sm">Non lavorativo</span>
          </div>
        ))}
        {blocks.map((block) => (
          <div className="absolute left-1.5 right-1.5 overflow-hidden rounded-lg border border-amber-300 px-2.5 py-1.5 text-[10px] font-black text-amber-950" key={block.id} style={{ ...itemPosition(block.starts_at, block.ends_at), background: "repeating-linear-gradient(135deg, #fffbeb 0, #fffbeb 8px, #fde68a 8px, #fde68a 11px)" }}>
        ...
        {appointments.map((item) => {
          ...
          return (
            <div
              className="absolute left-1.5 right-1.5 overflow-hidden rounded-lg border px-2.5 py-1.5 text-[10px] font-black shadow-sm"
              key={item.id}
              style={{
                ...itemPosition(item.starts_at, item.ends_at),
                background: confirmed ? `linear-gradient(135deg, ${color}, color-mix(in srgb, ${color} 72%, white))` : palette?.background,
                borderColor: confirmed ? undefined : palette?.border,
                color: confirmed ? "#fff" : palette?.text,
              }}
              title={`${time(item.starts_at)}–${time(item.ends_at)} · ${item.customer_name} · ${item.service_name} · ${appointmentStatusLabel(item.status)}`}
            >
              <span>{time(item.starts_at)}–{time(item.ends_at)}</span>
              <strong className="ml-2 truncate uppercase">{item.customer_name}</strong>
              <span className="ml-2 truncate font-semibold opacity-80">· {item.service_name}</span>
            </div>
          );
        })}
      </div>
...
export default function StaffDetailPage() {
  ...
  const [agendaDate, setAgendaDate] = useState(() => isoDateLocal(new Date()));
  ...
  useEffect(() => {
    if (!salon || tab !== "agenda") return;
    const dayStart = new Date(`${agendaDate}T00:00:00`);
    const dayEnd = new Date(dayStart.getTime() + 86_400_000);
    setAgendaLoading(true);
    void fetch(`${api}/api/salons/${salon.id}/calendar-events?staffId=${staffId}&from=${dayStart.toISOString()}&to=${dayEnd.toISOString()}`, { credentials: "include" })
      .then((response) => response.ok ? response.json() : { appointments: [], availability_blocks: [] })
      .then((data: { appointments?: AgendaAppointment[]; availability_blocks?: AgendaBlock[] }) => {
        setAgendaAppointments(data.appointments ?? []);
        setAgendaBlocks(data.availability_blocks ?? []);
      })
      .catch(() => setError("Agenda del collaboratore non disponibile."))
      .finally(() => setAgendaLoading(false));
  }, [salon?.id, staffId, tab, agendaDate]);
  ...
  const agendaDayLabel = useMemo(() => new Date(`${agendaDate}T00:00:00`).toLocaleDateString("it-IT", { dateStyle: "full" }), [agendaDate]);
          ...
          {tab === "agenda" && (
            <article className="rounded-2xl border border-[#e8dfe4] bg-white p-5 shadow-sm">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  <h2 className="flex items-center gap-2 font-bold text-stone-950"><CalendarDays aria-hidden="true" className="size-4 text-[#792f59]" />Agenda</h2>
                  <p className="mt-1 text-xs text-stone-500">{agendaDayLabel} · vista rapida della giornata, come nell’App Staff.</p>
                </div>
                <DateField aria-label="Vai a una data" onChange={setAgendaDate} value={agendaDate} />
              </div>
              <div className="mt-4">
                {agendaLoading ? <div className="h-64 animate-pulse rounded-xl bg-stone-100" /> : (
                  agendaAppointments.length === 0 && agendaBlocks.length === 0 ? (
                    <p className="rounded-xl bg-stone-50 p-6 text-center text-sm font-semibold text-stone-500">Nessun appuntamento o assenza in questa data.</p>
                  ) : (
                    <StaffAgendaTimeline
                      appointments={agendaAppointments}
                      blocks={agendaBlocks}
                      color={member.color}
                      day={new Date(`${agendaDate}T00:00:00`)}
