...
export default function CustomerPage({ params }: { params: Promise<{ customerId: string }> }) {
  ...
  useEffect(() => { void load().catch((reason: Error) => setError(reason.message)); }, [customerId, salon]);
  useEffect(() => {
    if (!salon) return;
    void fetch(`${api}/api/salons/${salon.id}/customers/tags`, { credentials: "include" }).then((response) => response.ok ? response.json() : []).then(setTags);
  }, [salon]);
  ...
  async function toggleBlocked() {
    ...
    const response = await fetch(`${api}/api/salons/${salon.id}/customers/${customerId}/block`, {
      method: "PATCH", credentials: "include", headers: { "content-type": "application/json" }, body: JSON.stringify({ blocked: !customer.blocked }),
    });
  ...
  async function createAppointment(formData: FormData) {
    ...
    const response = await fetch(`${api}/api/salons/${salon.id}/appointments`, {
      ...
      body: JSON.stringify({ customer_id: customerId, service_id: formData.get("service_id"), staff_id: formData.get("staff_id"), starts_at: new Date(String(formData.get("starts_at"))).toISOString(), notes: formData.get("notes") || undefined }),
    });
    if (!response.ok) {
      setError(response.status === 409 ? "Orario già occupato." : "Appuntamento non creato.");
      return;
    }
  ...
  return <main className="min-h-screen bg-[#f7f4f2] p-4 md:p-8"><div className="mx-auto max-w-6xl">
    <header className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-[#7b3159]">Profilo cliente</p><h1 className="mt-2 text-3xl font-bold">{customer.fullName}</h1><p className="mt-1 text-sm text-stone-500">{customer.email ?? "Nessuna email"} · {customer.phone ?? "Nessun telefono"}</p></div><div className="flex gap-2"><button onClick={() => void toggleBlocked()} className={`rounded-xl px-4 py-3 text-sm font-bold ${customer.blocked ? "bg-red-100 text-red-700" : "border border-stone-300"}`}>{customer.blocked ? "Sblocca cliente" : "Blocca cliente"}</button><button onClick={() => void openAppointment()} className="rounded-xl bg-[#7b3159] px-4 py-3 text-sm font-bold text-white">Nuovo appuntamento</button></div></header>
    ...
    <div className="mt-7 grid gap-5 lg:grid-cols-[.8fr_1.2fr]">
      <section className="space-y-5">
        ...
        <article className="rounded-2xl bg-white p-5 shadow-sm"><h2 className="font-bold">Tag</h2><div className="mt-4 flex flex-wrap gap-2">{Array.from(new Set([...tags, ...customer.tags])).map((tag) => { const selected = customer.tags.includes(tag); return <button key={tag} onClick={() => void patch({ tags: selected ? customer.tags.filter((item) => item !== tag) : [...customer.tags, tag] }).then(() => setCustomer((current) => current ? { ...current, tags: selected ? current.tags.filter((item) => item !== tag) : [...current.tags, tag] } : current))} className={`rounded-full px-3 py-2 text-xs font-bold ${selected ? "bg-[#7b3159] text-white" : "bg-stone-100"}`}>{tag}</button>; })}</div><input placeholder="Nuovo tag + Invio" onKeyDown={(event) => { if (event.key === "Enter" && event.currentTarget.value.trim()) { event.preventDefault(); const next = [...customer.tags, event.currentTarget.value.trim()]; void patch({ tags: next }); setCustomer({ ...customer, tags: next }); event.currentTarget.value = ""; } }} className="mt-3 w-full rounded-xl border p-3 text-sm" /></article>
