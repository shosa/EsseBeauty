...
export function AppRail({
  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-40 hidden w-[76px] flex-col items-center border-r border-white/10 bg-[var(--esse-rail,#25161f)] px-2 py-3 text-white shadow-[8px_0_28px_rgb(45_29_39_/_0.12)] md:flex">
      <Link aria-label="EsseBeauty Home" className="grid size-12 place-items-center rounded-xl bg-white text-[var(--esse-rail,#25161f)] shadow-sm transition hover:bg-white/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white" href="/" onClick={onNavigate}>
      ...
      <nav aria-label="App fissate" className="mt-4 flex flex-1 flex-col items-center gap-1.5">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-1.5">
          {pinned.map((app) => <Link aria-current={activePath(pathname, app.href) ? "page" : undefined} aria-label={app.label} className={`grid size-12 place-items-center rounded-xl transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white ${activePath(pathname, app.href) ? "bg-white text-[var(--esse-mulberry,#543147)] shadow-md" : "text-white/65 hover:bg-white/10 hover:text-white"}`} href={app.href} key={app.key} onClick={onNavigate} title={app.label}><app.icon /></Link>)}
        </div>
      </nav>
      <div className="flex flex-col items-center gap-1.5">
        <Link aria-label="Apri centro notifiche" className="relative grid size-12 place-items-center rounded-xl text-white/65 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white hover:bg-white/10 hover:text-white" href="/notifications" onClick={onNavigate}><BellIcon />{unreadCount > 0 && <span className="absolute right-1 top-1 grid size-5 place-items-center rounded-full bg-red-600 text-[10px] font-black">{Math.min(unreadCount, 9)}</span>}</Link>
        ...
        <button aria-label="Esci" className="group relative grid size-12 place-items-center rounded-xl bg-white/10 text-xs font-black text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white hover:bg-white hover:text-[var(--esse-mulberry,#543147)]" onClick={() => { onNavigate(); logout(); }} title="Esci" type="button"><span className="group-hover:hidden">{initials}</span><LogoutIcon className="hidden group-hover:block" /></button>
      </div>
    </aside>
  );
}
