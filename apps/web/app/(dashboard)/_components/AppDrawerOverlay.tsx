...
export function AppDrawerOverlay({ apps, badgeCounts, onClose, open }: { apps: readonly AppDefinition[]; badgeCounts?: Record<string, number>; onClose(): void; open: boolean }) {
  ...
  return (
    <div className="fixed inset-y-0 left-0 right-0 z-[100] bg-[#1c1118]/55 backdrop-blur-sm md:left-[76px]" onMouseDown={onClose}>
      <section
        aria-label="Tutte le app"
        aria-modal="true"
        className="esse-app-drawer-overlay absolute inset-y-0 left-0 w-full overflow-y-auto px-4 py-6 text-white shadow-[28px_0_90px_rgb(24_12_19_/_0.38)] md:w-[min(1120px,calc(100vw-76px))] md:px-8 lg:px-10"
        onMouseDown={(event) => event.stopPropagation()}
        ref={drawerRef}
        role="dialog"
        style={{
          background:
            "radial-gradient(circle at 18% 0%, var(--esse-drawer-glow, #98577c) 0%, transparent 35%), linear-gradient(145deg, var(--esse-drawer-start, #34202d) 0%, var(--esse-drawer-mid, #5f2948) 58%, var(--esse-drawer-end, #7e3a61) 100%)",
        }}
      >
        <header className="sticky top-0 z-10 -mx-1 flex items-start gap-4 rounded-xl bg-[var(--esse-ink,#25161f)]/88 px-4 py-4 shadow-lg backdrop-blur-xl">
