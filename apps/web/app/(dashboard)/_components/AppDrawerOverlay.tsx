...
export function AppDrawerOverlay({ apps, onClose, open }: { apps: readonly AppDefinition[]; onClose(): void; open: boolean }) {
  ...
  return (
    <div className="fixed inset-y-0 left-0 right-0 z-[100] bg-[#1c1118]/55 backdrop-blur-sm md:left-[76px]" onMouseDown={onClose}>
      <section aria-label="Tutte le app" aria-modal="true" className="esse-app-drawer-overlay absolute inset-y-0 left-0 w-full overflow-y-auto bg-[radial-gradient(circle_at_18%_0%,#98577c_0%,transparent_35%),linear-gradient(145deg,#34202d_0%,#5f2948_58%,#7e3a61_100%)] px-4 py-6 text-white shadow-[28px_0_90px_rgb(24_12_19_/_0.38)] md:w-[min(1120px,calc(100vw-76px))] md:px-8 lg:px-10" onMouseDown={(event) => event.stopPropagation()} role="dialog">
        <header className="sticky top-0 z-10 -mx-1 flex items-start gap-4 rounded-2xl bg-[#402334]/88 px-4 py-4 shadow-lg backdrop-blur-xl">
          <div className="min-w-0 flex-1"><p className="text-[10px] font-black uppercase tracking-[.2em] text-white/50">Workspace EsseBeauty</p><h1 className="mt-1 text-2xl font-black tracking-[-.035em]">Le tue app</h1><p className="mt-1 text-xs text-white/55">Apri uno strumento senza lasciare il contesto corrente.</p></div>
        ...
        <label className="relative mx-auto mt-5 block max-w-2xl"><Search className="pointer-events-none absolute left-4 top-1/2 size-4 -translate-y-1/2 text-stone-400" /><span className="sr-only">Cerca app</span><input autoFocus className="min-h-12 w-full rounded-2xl border border-white/20 bg-white/95 py-3 pl-11 pr-4 text-sm font-semibold text-stone-900 shadow-[0_14px_34px_rgb(26_13_21_/_0.2)] outline-none placeholder:text-stone-400 focus:border-[#e9b8d2] focus:ring-4 focus:ring-white/15" onChange={(event) => setQuery(event.target.value)} placeholder="Cerca app…" type="search" value={query} /></label>

        {filtered.length === 0 ? <div className="mx-auto mt-10 max-w-xl rounded-2xl bg-white p-4 text-stone-900"><EmptyState description="Prova a cercare un altro strumento." title="Nessuna app trovata" /></div> : <div className="mt-7 space-y-7 pb-8">{APP_DOMAINS.map((domain) => {
