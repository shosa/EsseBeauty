...
export function WorkspaceTopbar({ actions, app, onNotificationsOpen, onSearchOpen, pathname, tabs, unreadCount }: { actions: readonly AppQuickAction[]; app?: AppDefinition; onNotificationsOpen(): void; onSearchOpen(): void; pathname: string; tabs: readonly AppTab[]; unreadCount: number }) {
  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-30 border-b border-stone-200 bg-white/95 backdrop-blur md:left-[76px]">
      <div className="flex h-16 items-center gap-3 px-3 md:px-5">
        <Link aria-label="Apri tutte le app" className="grid size-11 place-items-center rounded-xl border border-stone-200 text-[#792f59] md:hidden" href="/apps"><ModuleIcon /></Link>
