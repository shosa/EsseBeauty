  ...
  return (
    <header className="fixed left-0 right-0 top-0 z-30 border-b border-stone-200 bg-white/95 backdrop-blur md:left-[76px]">
      <div className="flex h-16 items-center gap-3 px-3 md:px-5">
        <button aria-label="Apri tutte le app" className="grid size-11 place-items-center rounded-xl border border-stone-200 text-[var(--esse-mulberry,#543147)] md:hidden" onClick={onAppsOpen} type="button"><ModuleIcon /></button>
        ...
        <div aria-label="Azioni rapide" className="ml-auto flex shrink-0 items-center gap-2">
          <button className="hidden min-h-10 w-[min(24rem,30vw)] rounded-xl border border-stone-200 bg-stone-50 px-4 text-left text-sm text-stone-500 lg:block" onClick={onSearchOpen} type="button">Cerca cliente, appuntamento, servizio… <kbd className="float-right">Ctrl+K</kbd></button>
          {action && <Link className="hidden min-h-10 items-center rounded-xl bg-[var(--esse-mulberry,#543147)] px-4 text-sm font-semibold text-white hover:bg-[var(--esse-berry,#b85888)] sm:inline-flex" href={action.href}>{action.label}</Link>}
          <button aria-label="Cerca" className="grid size-11 place-items-center rounded-xl border border-stone-200 text-stone-600 lg:hidden" onClick={onSearchOpen} type="button"><Search aria-hidden="true" className="size-5" /></button>
          ...
          <button aria-label="Apri notifiche" className="relative grid size-11 place-items-center rounded-xl border border-stone-200 text-[var(--esse-mulberry,#543147)] hover:bg-[var(--esse-petal,#f2e1eb)]" onClick={onNotificationsOpen} type="button"><BellIcon />{unreadCount > 0 && <><span aria-hidden="true" className="absolute -right-1 -top-1 size-5 animate-ping rounded-full bg-[var(--esse-berry,#b85888)] opacity-75" /><span className="absolute -right-1 -top-1 grid size-5 place-items-center rounded-full bg-[var(--esse-mulberry,#543147)] text-[10px] font-black text-white">{Math.min(unreadCount, 9)}</span></>}</button>
        </div>
      </div>
