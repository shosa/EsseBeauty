...
export function MobileAppNavigation({ apps, pathname }: { apps: readonly AppDefinition[]; pathname: string }) {
  return <nav aria-label="Navigazione mobile" className="fixed inset-x-0 bottom-0 z-30 grid grid-cols-4 border-t border-stone-200 bg-white/95 px-2 pb-[env(safe-area-inset-bottom)] backdrop-blur md:hidden">{mobileDestinations.map((item) => item.href === "/apps" || apps.some((app) => app.href === item.href) ? <Link aria-current={pathname === item.href || (item.href !== "/" && pathname.startsWith(item.href)) ? "page" : undefined} aria-label={item.href === "/apps" ? "Apri tutte le app" : undefined} className={`flex min-h-16 flex-col items-center justify-center gap-1 text-[10px] font-semibold ${pathname === item.href || (item.href !== "/" && pathname.startsWith(item.href)) ? "text-[#792f59]" : "text-stone-500"}`} href={item.href} key={item.label}><item.icon /><span>{item.label}</span></Link> : <span aria-hidden="true" key={item.label} />)}</nav>;
}
