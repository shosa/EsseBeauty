...
function Card({ actions, bodyClassName = "", children, className = "", subtitle, title }: { actions?: ReactNode; bodyClassName?: string; children: ReactNode; className?: string; subtitle?: ReactNode; title?: ReactNode }) {
  return (
    <div className={`overflow-hidden rounded-2xl border border-[#e8dfe4] bg-white shadow-[0_10px_30px_rgb(45_29_39_/_0.055)] transition hover:-translate-y-0.5 hover:border-[#d7a6c1] hover:shadow-md ${className}`}>
          ...
          {!compareEnabled || !previousData ? (
            ...
            <>
              <div className="grid gap-3 sm:grid-cols-3">
                <div className="rounded-2xl border border-[#e8dfe4] bg-white p-4 shadow-[0_10px_30px_rgb(45_29_39_/_0.055)] transition hover:-translate-y-0.5 hover:border-[#d7a6c1] hover:shadow-md"><span className="text-[11px] font-black uppercase tracking-[.08em] text-stone-500">Incassato</span><div className="mt-1.5 flex items-baseline gap-2"><strong className="text-2xl font-black tnum">{euro(data?.summary.total_cents ?? 0)}</strong><span className="tnum text-xs text-stone-400 line-through">{euro(previousData.summary.total_cents)}</span></div><DeltaChip value={incassatoDelta} /></div>
                <div className="rounded-2xl border border-[#e8dfe4] bg-white p-4 shadow-[0_10px_30px_rgb(45_29_39_/_0.055)] transition hover:-translate-y-0.5 hover:border-[#d7a6c1] hover:shadow-md"><span className="text-[11px] font-black uppercase tracking-[.08em] text-stone-500">Margine netto</span><div className="mt-1.5 flex items-baseline gap-2"><strong className="text-2xl font-black tnum">{euro(overview?.summary.gross_margin_cents ?? 0)}</strong><span className="tnum text-xs text-stone-400 line-through">{euro(previousOverview?.summary.gross_margin_cents ?? 0)}</span></div><DeltaChip value={margineDelta} /></div>
                <div className="rounded-2xl border border-[#e8dfe4] bg-white p-4 shadow-[0_10px_30px_rgb(45_29_39_/_0.055)] transition hover:-translate-y-0.5 hover:border-[#d7a6c1] hover:shadow-md"><span className="text-[11px] font-black uppercase tracking-[.08em] text-stone-500">Scontrino medio</span><div className="mt-1.5 flex items-baseline gap-2"><strong className="text-2xl font-black tnum">{euro(data?.summary.average_cents ?? 0)}</strong><span className="tnum text-xs text-stone-400 line-through">{euro(previousData.summary.average_cents)}</span></div><DeltaChip value={scontrinoDelta} /></div>
              </div>
