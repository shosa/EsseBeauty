...
function Row({ item, canEdit, onEdit, onToggle }: { item: Service; canEdit: boolean; onEdit(): void; onToggle(): void }) {
  const sortable = useSortable({ id: item.id, disabled: !canEdit });
  return <article ref={sortable.setNodeRef} style={{ transform: CSS.Transform.toString(sortable.transform), transition: sortable.transition }} className={`grid grid-cols-[32px_1fr_auto] items-center gap-3 border-b border-stone-100 px-4 py-4 last:border-0 ${item.active ? "" : "opacity-55"}`}>
    ...
    <div className="flex items-center gap-3"><strong className="text-stone-800">{formatPrice(item.priceCents, "it-IT")}</strong><Switch checked={item.active} disabled={!canEdit} onCheckedChange={onToggle} /></div>
  </article>;
}
