...
export default function CalendarPage() {
  ...
  const canDelete = hasPermission(PERMISSION_KEYS.CALENDAR_DELETE);
  ...
  async function createAppointment(formData: FormData) {
    ...
    if (!response.ok) {
      setError(response.status === 409 ? "L'orario selezionato e gia occupato." : "L'appuntamento non e stato creato.");
      return;
    }
    setCreateOpen(false);
    await load();
  ...
  async function updateStatus(status: string) {
    ...
    setError("");
    const response = await fetch(`${api}/api/salons/${salon.id}/appointments/${selected.id}`, {
      method: "PATCH",
      credentials: "include",
      headers: { "content-type": "application/json" },
    ...
    if (!response.ok) {
      setError("Lo stato dell'appuntamento non e stato aggiornato.");
      return;
    }
  ...
  async function deleteAppointment() {
    if (!selected || !salon || !window.confirm(`Eliminare l'appuntamento di ${selected.customer_name}?`)) return;
    setError("");
    const response = await fetch(`${api}/api/salons/${salon.id}/appointments/${selected.id}`, {
      method: "DELETE",
      credentials: "include",
    ...
    if (!response.ok) {
      setError(response.status === 403 ? "Non hai il permesso di eliminare questo appuntamento." : "L'appuntamento non e stato eliminato.");
      return;
    }
    setSelected(undefined);
    await load();
      ...
      <div className="mx-auto max-w-[1500px]">
        ...
        <div className="overflow-x-auto rounded-2xl border border-stone-200 bg-white shadow-sm">
          <div className="grid min-w-[840px] grid-cols-7">
            {days.map((day) => (
              <section key={day.toISOString()} className="min-h-[640px] border-r border-stone-100 last:border-0">
                <header className="sticky top-0 border-b border-stone-100 bg-white p-3 text-center">
                  <p className="text-xs uppercase text-stone-400">{day.toLocaleDateString("it-IT", { weekday: "short" })}</p>
        ...
        {selected && (
          <aside className="fixed inset-y-0 right-0 z-40 w-full max-w-md overflow-y-auto bg-white p-6 shadow-2xl">
            <button onClick={() => setSelected(undefined)} className="float-right">Chiudi</button>
            <p className="text-sm font-bold uppercase tracking-wider text-rose-700">Appuntamento</p>
            <h2 className="mt-2 text-2xl font-bold">{selected.customer_name}</h2>
            <p className="mt-2 text-stone-600">{selected.service_name} con {selected.staff_name}</p>
            <p className="mt-5 rounded-xl bg-stone-100 p-4">{new Date(selected.starts_at).toLocaleString("it-IT", { dateStyle: "full", timeStyle: "short" })}</p>
            {canCreate ? (
              <div className="mt-6 grid grid-cols-2 gap-2">
                {statusActions.map((action) => (
                  <button key={action.status} onClick={() => void updateStatus(action.status)} className="rounded-xl border border-stone-300 px-3 py-3 text-sm font-semibold">{action.label}</button>
                ))}
              </div>
            ) : (
              <p className="mt-5 rounded-xl bg-amber-50 p-4 text-sm text-amber-800">Calendario in sola lettura.</p>
            )}
            {canDelete && <button onClick={() => void deleteAppointment()} className="mt-4 w-full rounded-xl border border-red-200 px-4 py-3 text-sm font-bold text-red-700">Elimina appuntamento</button>}
          </aside>
        )}

        {createOpen && (
          <div className="fixed inset-0 z-50 grid place-items-center bg-black/35 p-4">
            <form action={createAppointment} className="grid w-full max-w-lg gap-4 rounded-[2rem] bg-white p-6 shadow-2xl">
              <div className="flex justify-between">
                <h2 className="text-xl font-bold">Nuovo appuntamento</h2>
                <button type="button" onClick={() => setCreateOpen(false)}>Chiudi</button>
              </div>
              <select required name="customer_id" className="min-h-11 rounded-xl border bg-white px-3">
                <option value="">Seleziona cliente</option>
                {customers.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
              </select>
              <select required name="service_id" className="min-h-11 rounded-xl border bg-white px-3">
                <option value="">Seleziona servizio</option>
                {services.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
              </select>
              <select required name="staff_id" className="min-h-11 rounded-xl border bg-white px-3">
                <option value="">Seleziona collaboratore</option>
                {staff.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
              </select>
              <input required type="datetime-local" name="starts_at" className="min-h-11 rounded-xl border px-3" />
              <textarea name="notes" placeholder="Note interne" className="rounded-xl border p-3" />
              <button className="min-h-11 rounded-xl bg-[#402334] font-bold text-white">Crea appuntamento</button>
            </form>
          </div>
        )}
      </div>
