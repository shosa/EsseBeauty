...
export default function ReviewsPage() {
  ...
  return <main className="min-h-screen bg-[#f6f2f4] p-5 md:p-10"><div className="mx-auto max-w-6xl"><header><p className="text-xs font-bold uppercase tracking-[.2em] text-[#792f59]">Voce dei clienti</p><h1 className="mt-2 text-3xl font-bold">Recensioni</h1></header>
    ...
    <section className="mt-6 space-y-3">{items.map((item) => <article key={item.id} className="rounded-2xl bg-white p-5 shadow-sm"><div className="flex flex-wrap justify-between gap-3"><div><p className="text-[#a33d72]">{"★".repeat(item.rating)}<span className="text-stone-200">{"★".repeat(5-item.rating)}</span></p><h2 className="mt-1 font-bold">{item.customer_name}</h2></div><span className={`h-fit rounded-full px-3 py-1 text-xs ${item.published ? "bg-emerald-50 text-emerald-700" : "bg-stone-100"}`}>{item.published ? "Pubblicata" : "Privata"}</span></div><p className="mt-3 text-stone-600">{item.comment || "Nessun commento."}</p>{item.reply && <p className="mt-3 border-l-2 border-[#d9a5c2] pl-3 text-sm"><b>Risposta:</b> {item.reply}</p>}<button onClick={() => { setSelected(item); setReply(item.reply ?? ""); }} className="mt-4 font-semibold text-[#792f59]">Rispondi</button></article>)}</section>
