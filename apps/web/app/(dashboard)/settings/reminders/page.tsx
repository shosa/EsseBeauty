    ...
    <AppPage maxWidth="max-w-[1600px]">
      ...
      <SectionCard title="Regole promemoria">
        <div className="grid gap-5 md:grid-cols-2">
          ...
          <fieldset><legend className="font-semibold">Quando inviarli</legend><div className="mt-4 grid grid-cols-2 gap-3">{options.map((value) => <label key={value} className={`flex min-h-12 items-center rounded-xl border p-4 ${hours.includes(value) ? "border-[#792f59] bg-[#faf3f7]" : "border-stone-200"}`}><input type="checkbox" checked={hours.includes(value)} disabled={saving} onChange={() => { const nextHours = hours.includes(value) ? hours.filter((item) => item !== value) : [...hours, value]; setHours(nextHours); void save(whatsapp, email, nextHours); }} className="mr-2 size-5 accent-[#792f59]" />{value} ore prima</label>)}</div></fieldset>
