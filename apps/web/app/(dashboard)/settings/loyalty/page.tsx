...
export default function LoyaltySettingsPage() {
  ...
  async function toggleReward(reward: Reward) {
    if (!salon) return;
    const response = await fetch(`${api}/api/salons/${salon.id}/loyalty/rewards/${reward.id}`, { method: "PATCH", credentials: "include", headers: { "content-type": "application/json" }, body: JSON.stringify({ active: !reward.active }) });
    if (response.ok) await load();
  }
  async function removeReward(reward: Reward) {
    if (!salon || !window.confirm(`Eliminare il premio "${reward.name}"?`)) return;
    const response = await fetch(`${api}/api/salons/${salon.id}/loyalty/rewards/${reward.id}`, { method: "DELETE", credentials: "include" });
    if (response.ok) await load();
  }
  return <main className="min-h-screen bg-[#f6f2f4] p-5 md:p-10"><div className="mx-auto max-w-4xl"><p className="text-xs font-bold uppercase tracking-[.2em] text-[#792f59]">Modulo</p><h1 className="mt-2 text-3xl font-bold">Programma fedeltà</h1>
    ...
    <section className="mt-5 rounded-2xl bg-white p-6 shadow-sm"><h2 className="font-bold">Premi</h2><form action={addReward} className="mt-4 grid gap-3 md:grid-cols-[1fr_120px_1fr_auto]"><input required name="name" placeholder="Nome premio" className="rounded-xl border p-3" /><input required type="number" min={1} name="points" placeholder="Punti" className="rounded-xl border p-3" /><input name="description" placeholder="Descrizione" className="rounded-xl border p-3" /><button className="rounded-xl bg-stone-900 px-4 py-3 font-bold text-white">Aggiungi</button></form><div className="mt-5 space-y-3">{rewards.length === 0 && <p className="rounded-xl bg-stone-50 p-5 text-center text-sm text-stone-500">Nessun premio configurato.</p>}{rewards.map((reward) => <article key={reward.id} className={`flex flex-wrap items-center justify-between gap-4 rounded-xl border p-4 ${reward.active ? "" : "opacity-55"}`}><div><h3 className="font-bold">{reward.name}</h3><p className="text-sm text-stone-500">{reward.description ?? "Nessuna descrizione"}</p></div><div className="flex items-center gap-4"><strong>{reward.pointsRequired} pt</strong><button onClick={() => void toggleReward(reward)} className="text-sm font-semibold text-[#792f59]">{reward.active ? "Disattiva" : "Attiva"}</button><button onClick={() => void removeReward(reward)} className="text-sm font-semibold text-red-700">Elimina</button></div></article>)}</div></section>
  </div></main>;
}
