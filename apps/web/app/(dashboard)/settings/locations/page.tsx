...
import { AppPage, Button, EmptyState, FormField, InlineError, KpiStrip, PageHeader, SaveActionButton, SaveToast, SectionCard, Switch, Select} from "@esse-beauty/ui";
import { useAuth } from "../../../../lib/auth-context";

const api = process.env.NEXT_PUBLIC_API_URL ?? "";
const timezones = [
  ["Europe/Rome", "Roma — Europa centrale"],
  ["Europe/London", "Londra — Regno Unito"],
  ["Europe/Paris", "Parigi — Europa centrale"],
  ["Europe/Berlin", "Berlino — Europa centrale"],
  ["Europe/Madrid", "Madrid — Europa centrale"],
  ["Europe/Zurich", "Zurigo — Europa centrale"],
] as const;
  ...
  return (
    <AppPage maxWidth="max-w-[1600px]">
      ...
      <div className="mt-4 grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)]">
        <SectionCard icon={Building2} title="Sedi operative" subtitle="Seleziona la sede da configurare.">
          <div className="space-y-2">{locations.map((location) => <button aria-pressed={locationId === location.id} className={`flex min-h-12 w-full items-center gap-3 rounded-lg border p-3 text-left ${locationId === location.id ? "border-[var(--esse-mulberry,#543147)] bg-[var(--esse-petal,#f2e1eb)]" : "border-stone-200 hover:bg-stone-50"}`} key={location.id} onClick={() => setLocationId(location.id)} type="button"><Building2 aria-hidden="true" className="size-5 shrink-0 text-[var(--esse-mulberry,#543147)]" /><span className="min-w-0 flex-1"><strong className="block truncate">{location.name}</strong><small className="block truncate text-stone-500">{location.address || "Indirizzo da completare"}</small></span><span aria-label={`${cabinCount(location.id)} cabine`} className="flex items-center gap-1 text-xs font-bold text-stone-500"><DoorOpen aria-hidden="true" size={14} />{cabinCount(location.id)}</span></button>)}</div>
