...
export function FeedbackSystemPreview() {
  ...
  const [soundFeel, setSoundFeel] = useState<SoundFeel>("soft");
  ...
  function play(cue: CueName) {
    if (!soundPlayer.current) soundPlayer.current = createUISFX({ pack: soundFeel, volume: 0.42 });
