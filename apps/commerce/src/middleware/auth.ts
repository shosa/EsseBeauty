...
import { hashSessionToken, sessionCookieForClient } from "@esse-beauty/server-shared";
...
// Verifies the same session cookie against the same shared authSessions/users
// tables apps/api uses (see apps/api/src/middleware/auth.ts) — every service
// reads the one Postgres database, so there is no token/claims hand-off yet.
// The identity service planned for a later phase replaces this with locally
// verifiable signed tokens instead of a per-request session-table lookup.
