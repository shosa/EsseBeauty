...
function productValues(input: Partial<ProductInput>, salonId: string, partial = false) {
  const values = {
    ...(input.active !== undefined && { active: input.active }), ...(input.allow_negative_stock !== undefined && { allowNegativeStock: input.allow_negative_stock }), ...(input.average_cost_cents !== undefined && { averageCostCents: input.average_cost_cents }), ...(input.barcode !== undefined && { barcode: input.barcode }), ...(input.category !== undefined && { category: input.category }), ...(input.cost_cents !== undefined && { costCents: input.cost_cents }), ...(input.internally_consumable !== undefined && { internallyConsumable: input.internally_consumable }), ...(input.item_type !== undefined && { itemType: input.item_type }), ...(input.last_cost_cents !== undefined && { lastCostCents: input.last_cost_cents }), ...(input.low_stock_threshold !== undefined && { lowStockThreshold: input.low_stock_threshold }), ...(input.name !== undefined && { name: input.name.trim() }), ...(input.preferred_supplier !== undefined && { preferredSupplier: input.preferred_supplier }), ...(input.preferred_supplier_id !== undefined && { preferredSupplierId: input.preferred_supplier_id }), ...(input.reorder_quantity !== undefined && { reorderQuantity: input.reorder_quantity }), ...(input.sellable !== undefined && { sellable: input.sellable }), ...(input.sku !== undefined && { sku: input.sku }), ...(input.stock_quantity !== undefined && { stockQuantity: input.stock_quantity }), ...(input.supplier !== undefined && { supplier: input.supplier }), ...(input.track_stock !== undefined && { trackStock: input.track_stock }), ...(input.unit !== undefined && { unit: input.unit }), ...(input.unit_price_cents !== undefined && { unitPriceCents: input.unit_price_cents }), ...(input.unit_scale !== undefined && { unitScale: input.unit_scale }),
  };
