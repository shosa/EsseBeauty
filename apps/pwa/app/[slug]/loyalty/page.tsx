    ...
    <main className="min-h-screen bg-[#faf8f4] px-4 py-8">
      <section className="mx-auto max-w-lg">
        <header>
          ...
          <p className="mt-2 text-sm text-stone-500">Consulta saldo, premi e movimenti del tuo account.</p>
        </header>
        ...
        {data && (
          <div className="animate-reveal mt-6 space-y-5">
            ...
            <section className="rounded-3xl border border-stone-200 bg-white p-6">
              ...
              <div className="mt-4 space-y-3">
                {data.rewards.map((reward, index) => (
                  <article className={`animate-reveal flex justify-between gap-4 rounded-2xl border border-stone-200 bg-white p-4 ${reward.available ? "" : "opacity-55"}`} key={reward.id} style={{ animationDelay: `${Math.min(index, 6) * 40}ms` }}>
                    <div><b>{reward.name}</b><p className="text-sm text-stone-500">{reward.description}</p><span className="mt-2 inline-block text-xs font-bold" style={{ color: reward.available ? "#0e7c59" : "#78716c" }}>{reward.available ? "Disponibile in salone" : `Mancano ${reward.pointsRequired - data.balance} punti`}</span></div>
            ...
            <section className="rounded-3xl border border-stone-200 bg-white p-6">
              ...
              <div className="mt-4 border-l-2 border-stone-200 pl-5">
                {data.history.map((item) => (
                  <article key={item.id} className="relative border-b border-stone-100 py-4 last:border-0">
                    <span className="absolute -left-[27px] top-6 size-3 rounded-full ring-4 ring-[#faf8f4]" style={{ background: primary }} />
        ...
        {authStatus === "anonymous" && (
          <div className="animate-reveal mt-6 rounded-3xl border border-stone-200 bg-white p-6 text-center">
