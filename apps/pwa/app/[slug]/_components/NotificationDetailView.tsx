...
import { CustomerAuthOverlay } from "./CustomerAuthOverlay";
...
/**
 * Shared shell for a push notification's dedicated landing page — fetches the
 * customerAppMessages record by id and renders it full-screen, styled per
 * notification kind via the icon/label props. Reused by the generic message
 * page and the per-kind reminder/campaign pages so they open reliably tied to
 * their own content instead of a "read this, then navigate" detour.
 */
export function NotificationDetailView({ authSubtitle, ctaLabel, icon: Icon }: {
  authSubtitle: string;
  ctaLabel: string;
  icon: LucideIcon;
}) {
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#faf8f4] p-5">
      {authStatus === "anonymous" && (
        <CustomerAuthOverlay accent={accent} primary={primary} subtitle={authSubtitle} />
      )}
      {authStatus === "authenticated" && (
        ...
        >
          ...
          <span className="mx-auto grid size-14 place-items-center rounded-2xl text-white" style={{ background: primary }}><Icon className="size-6" /></span>
          {loading ? (
          ...
          <button className="mt-7 min-h-12 w-full rounded-full font-black text-white" onClick={done} style={{ background: primary }} type="button">{ctaLabel}</button>
        </motion.section>
      )}
    </main>
  );
