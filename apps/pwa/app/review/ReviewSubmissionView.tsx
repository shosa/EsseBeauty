...
export function ReviewSubmissionView() {
  ...
  if (state.submitted) {
    return <main className="grid min-h-screen place-items-center bg-[#faf8f4] p-5"><section className="max-w-md rounded-3xl border border-stone-200 bg-white p-8 text-center"><div className="text-5xl text-[#b8862e]">★★★★★</div><h1 className="mt-5 text-3xl font-bold">Grazie del tuo tempo</h1><p className="mt-3 text-stone-600">La tua esperienza aiuta il salone a migliorare.</p></section></main>;
  }

  return <main className="min-h-screen bg-[#faf8f4] px-4 py-10"><section className="mx-auto max-w-lg overflow-hidden rounded-3xl border border-stone-200 bg-white">
    <header className="bg-stone-950 p-7 text-white"><p className="text-xs font-bold uppercase tracking-[.2em] text-stone-400">{summary?.salon_name ?? "Recensione"}</p><h1 className="mt-2 text-3xl font-bold">Com&apos;è andata?</h1><p className="mt-3 text-sm text-stone-300">{summary ? `${summary.service_name} · ${new Date(summary.starts_at).toLocaleDateString("it-IT")}` : loading ? "Caricamento…" : "Invito non disponibile"}</p></header>
    <div className="space-y-6 p-7">
      {summary && <><div><p className="mb-3 font-semibold">La tua valutazione</p><div className="flex justify-between">{[1, 2, 3, 4, 5].map((star) => <button aria-label={`${star} stelle`} className={`rounded-2xl px-2 text-4xl transition hover:-translate-y-0.5 ${star <= state.rating ? "bg-amber-50 text-[#b8862e]" : "text-stone-200 hover:bg-stone-50"}`} key={star} onClick={() => dispatch({ rating: star, type: "rating" })} type="button">★</button>)}</div></div>
      ...
      {summary && <button className="min-h-13 w-full rounded-full bg-stone-950 font-bold text-white transition hover:-translate-y-0.5 disabled:opacity-45" disabled={!state.rating || state.submitting} onClick={() => void submit()} type="button">{state.submitting ? "Invio in corso…" : "Invia recensione"}</button>}
    </div>
  </section></main>;
}
