...
export function ConsentSigningView() {
  ...
  if (signing.signed) {
    return (
      <main className="grid min-h-screen place-items-center bg-[#faf8f4] p-5">
        <section className="w-full max-w-lg rounded-3xl border border-stone-200 bg-white p-8 text-center">
  ...
  return (
    <main className="min-h-screen bg-[#faf8f4] px-4 py-8 sm:py-12">
      <section className="mx-auto max-w-3xl overflow-hidden rounded-3xl border border-stone-200 bg-white">
        <header className="bg-stone-950 p-6 text-white sm:p-8">
          <p className="text-xs font-bold uppercase tracking-[.2em] text-stone-400">{documentView?.salon.name ?? "Documento di consenso"}</p>
            ...
            <>
              ...
              <div className="mt-6 grid gap-4">
                <label className="text-sm font-bold text-stone-700">Nome e cognome
                  <input autoComplete="name" className="mt-2 min-h-12 w-full rounded-xl border border-stone-300 bg-white px-4 font-normal text-stone-950 outline-none focus:border-stone-950 focus:ring-4 focus:ring-stone-950/10" onChange={(event) => dispatchSigning({ field: "signerName", type: "change", value: event.target.value })} value={signing.signerName} />
                </label>
                <label className="flex items-start gap-3 rounded-2xl border border-stone-200 bg-stone-50 p-4 text-sm font-semibold leading-6 text-stone-800">
                  <input checked={signing.accepted} className="mt-1 size-5 shrink-0 accent-stone-950" onChange={(event) => dispatchSigning({ field: "accepted", type: "change", value: event.target.checked })} type="checkbox" />
                ...
                <button className="min-h-13 rounded-full bg-stone-950 px-5 font-bold text-white transition hover:-translate-y-0.5 disabled:cursor-not-allowed disabled:opacity-45" disabled={signing.submitting || !signing.accepted || !signing.signerName.trim()} onClick={() => void sign()} type="button">
