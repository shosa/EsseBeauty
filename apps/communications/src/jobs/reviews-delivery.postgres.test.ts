...
vi.mock("@esse-beauty/comms-contracts", async (importOriginal) => ({
  ...
  sendEmailFromDb: async (_db: DrizzleDB, to: string, subject: string, html: string) => senders.sendEmail(to, subject, html),
}));

import { ensureReviewInvitation, enqueueCommunication, type ReviewRequestJob } from "@esse-beauty/comms-contracts";
import {
