...
import { scheduleReviewInvitation } from "@esse-beauty/comms-contracts";
...
postgresSuite("review queue recovery with PostgreSQL", () => {
  ...
  it("recovers a durable invitation after queue add failure and reuses a stable job id", async () => {
    ...
    try {
      ...
      await expect(scheduleReviewInvitation(db, appointmentId, failingQueue)).rejects.toThrow("redis unavailable");
      ...
      await scheduleReviewInvitation(db, appointmentId, recoveryQueue);
