...
import { enqueueCommunication, type CommunicationOutboxJob } from "@esse-beauty/comms-contracts";

import { testDatabaseUrl } from "../test/postgres.js";
import { processCommunicationOutbox } from "./communications.js";
...
postgresSuite("durable WhatsApp outbox with PostgreSQL", () => {
  ...
  it("emits a campaign-status-refresh event only after the outbox provider accepts its recipient", async () => {
    const data = await fixture();
    const queue = { add: vi.fn(async () => undefined) };
    const scheduleCampaignStatusRefresh = vi.fn(async () => undefined);
    try {
      ...
      expect(scheduleCampaignStatusRefresh).toHaveBeenCalledWith(campaign.id);
    } finally {
      await db.delete(salons).where(eq(salons.id, data.salonId));
    }
  });
  ...
  it("reactivates an exhausted campaign recipient outbox for an explicit stable-idempotency retry", async () => {
    ...
    try {
      ...
      expect(scheduleCampaignStatusRefresh).toHaveBeenCalledWith(campaign.id);
      scheduleCampaignStatusRefresh.mockClear();
