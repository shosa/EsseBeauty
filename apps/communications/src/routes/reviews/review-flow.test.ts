...
import { hashSessionToken, issuePublicToken } from "@esse-beauty/server-shared";
import { ensureReviewInvitation } from "@esse-beauty/comms-contracts";
...
postgresSuite("secure review lifecycle with PostgreSQL", () => {
  ...
  async function ensureInvitation(appointmentId: string, options?: { expiresAt?: Date }): Promise<{ id: string }> {
    return ensureReviewInvitation(db, appointmentId, options);
  }
