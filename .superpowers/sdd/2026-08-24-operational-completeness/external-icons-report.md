...
Second source scan converted the remaining functional SVGs in the shared UI close controls, dashboard plus action, clients search, sales customer card, appointment status/close/remove controls, and audit activity categories. The scan now reports no `<svg>` occurrences under `apps` or `packages`; remaining visual marks are CSS/brand assets rather than hand-authored icon SVGs.
